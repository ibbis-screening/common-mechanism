...
for res in glob.glob('*.screen'):
    ...
    with open(res) as f:
        ...
        matching_fungi.append[s for s in lines if "found in only regulated organisms: FLAG (oomycetes)" in s]
        matching_fungi.append[s for s in lines if "found in only regulated organisms: FLAG (ascomycetes)" in s]
